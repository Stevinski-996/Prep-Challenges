...
const recursionPattern = (int1, int2, result = [int1]) => {
    const next = result[result.length - 1] - int2;
  
    if (next <= 0) {
      result.push(next);  
      const reverse = result.slice(0, -1).reverse(); 
      return [...result, ...reverse]; 
    } else {
      result.push(next);
      return recursionPattern(int1, int2, result); 
    }
  };
...
const filterLinks = (str) => {
    const match = str.match(/href="https?:\/\/([^"]+?\.(?:com|org|net))"/i);
